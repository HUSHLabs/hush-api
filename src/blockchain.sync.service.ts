...
import {
    ERC20RecordSyncAdapter
} from "./blockchain.sync.adapters"
...
export class BlockchainSyncService extends BaseBlockchainSyncService {
    ...
    protected async getBlockchainSyncState(entity: string): Promise<BlockchainSyncState | null> {
        return await this.prismaService.blockchainSyncState.findFirst({ where: { entity } })
    }

    protected async updateBlockchainSyncState(syncState: BlockchainSyncState): Promise<void> {
